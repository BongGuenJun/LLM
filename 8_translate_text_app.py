...
example = {
    ...
    "영어":["How's the weather today?","Multimodal AI technology is gaining popularity recently."],
    "일본어":["今日の天気はどうですか?","最近、マルチモーダルAI技術が人気を集めている。"]
}      # 어떻게 번역해야할지에 대한 예시
...
def translate_text_chatgpt(text,src_lang,trg_lang):
    ...
    messages = [{"role":"system", "content":system_instruction}, *fewshot_messages, {"role":"user", "content":text}] #명령파트
    #print(messages)
...
def gradio_translate(text, src_lang, trg_lang):
    ...
    if src_lang == trg_lang:
        return "원본언어와 번역 언어가 같습니다. 다른 언어를 선택하세요"
...
with gr.Blocks() as demo:
    ...
    src_lang_dropdown = gr.Dropdown(choices=['한국어', '영어', '일본어'],
    value='영어', label='번역할 언어를 선택하세요')
    trg_lang_dropdown = gr.Dropdown(choices=['영어', '한국어', '일본어'],
    value='한국어', label='번역된 언어를 선택하세요')
